...
$sql1="SELECT fullname From users WHERE username = '".$more."'";
$result1 = mysqli_query($mysqli, $sql1);



if ($result1->num_rows >= 1) {
  $sql = "SELECT * From users WHERE username = '".$more."'";
...
if (isset($_POST['btn-chat'])) {
	$sql2 = "SELECT username From friend WHERE username='".$username."' AND friend='".$more."' ";
	$result2 = mysqli_query($mysqli,$sql2);
	$sql3 = "SELECT username From friend WHERE username='".$more."' AND friend='".$username."' ";
	$result3 = mysqli_query($mysqli,$sql3);
	if (!mysqli_num_rows($result2) && !mysqli_num_rows($result3)) {
		$mysqli->query("INSERT INTO friend (username, friend) VALUES ('$username', '$more')");
	}
  	header("Location: https://weunderstand.ru/friends.php");
}
      ...
      <div class="content">
                <div class="container">
                    ...
                    <div class="row d-flex justify-content-center centt" id="r1">
                        <div class="col-lg-3 justify-content-center d-flex" id="sometest">
                        <div class="text-center card-box">
                                <div class="member-card ">
                                  <div class="thumb-lg member-thumb mx-auto glx_img ">
                                    <img class="rounded-circle img-thumbnail center-block img-responsive img-thumbnail thumb96 mt" src="<?php echo $avatar;  ?>" alt="Contact">
                                </div>
                                <span  class="inline"><i class="fa fa-user" <?php if ($row['online'] == "yes"){ echo 'style="color: #2ade2a;"'; } else { echo 'style="color: grey;"'; } ?>></i></span>
                                <span  class="inline"><h4 class="pb" id="login"><?php echo $row['username']; ?></h4></span>
                          <p <?php if ($row['volunteer'] == "no"){ echo 'style="visibility:hidden;"'; } ?>>( Волонтёр )</p>
                          <form method="post">
                                 <button type="submit" class="btn btn-primary mt-3 btn-rounded waves-effect w-md waves-light btn-chat" name="btn-chat">Добавить в друзья</button>
                               </form>
                              </div>
                        ...
                        <div class="col-lg-5 d-flex">
                            <div class="text-center card-box">
                                <div class="member-card m_card">
                                    ...
                                    <div class="loginuser">
                                        <span>
                                            <i class="fa fa-birthday-cake"></i> Возраст:
                                        </span>
                                        <span>
                                            <h5 class="pb" id="login"><?php echo $age; ?></h5>
                                        </span>
                                        <br>
                                        <span>
                                        ...
                                        <span>
                                            <h5 class="pb" id="login"><?php echo $row['gender']; ?></h5>
                                        </span>
                                    </div>
                                    <div class="mv-lg loginuser">
